...
exports.authenticate = async (req, res, next) => {
    try {
        ...
        if (user.status !== 'ACTIVE') {
            logger.warn(`认证失败: 用户账号不可用 [用户ID: ${user._id}, 状态: ${user.status}]`);
        ...
        logger.info(`认证成功 [用户ID: ${user._id}, 用户名: ${user.username || user.name}, 角色: ${user.role}]`);
    ...
    return (req, res, next) => {
        try {
            ...
            if (!roles.includes(req.user.role)) {
                logger.warn(`授权失败: 权限不足 [用户ID: ${req.user._id}, 角色: ${req.user.role}, 需要角色: ${roles.join(', ')}]`);
                return res.status(403).json({
                    error: '权限不足',
                    message: '您没有权限执行此操作'
                });
            ...
            logger.info(`授权成功 [用户ID: ${req.user._id}, 角色: ${req.user.role}, 允许角色: ${roles.join(', ')}]`);
            
            // 继续处理请求
            next();
        } catch (error) {
            logger.error(`授权过程发生错误: ${error.message} [用户ID: ${req.user?._id}]`, error);
            res.status(500).json({
                error: '授权失败',
                message: error.message || '未知错误'
            });
    ...
    try {
        ...
        if (!user) {
            ...
            user = new User({
                ...
                icbcAccountId: icbcUserInfo.accountId,
            ...
            logger.info(`工行认证成功: 创建新用户 [工行用户ID: ${icbcUserId}, 本地用户ID: ${user._id}]`);
        } else {
            // 更新最后登录时间
            user.lastLogin = Date.now();
            await user.save();
            
            logger.info(`工行认证成功: 用户登录 [工行用户ID: ${icbcUserId}, 本地用户ID: ${user._id}]`);
        }
